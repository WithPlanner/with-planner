...
class EmailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.authBtn.setOnClickListener{
            ...
            RetrofitService.userService.checkValidEmail(email)?.enqueue(object : Callback<EmailAuth> {
                override fun onResponse(call: Call<EmailAuth>, response: Response<EmailAuth>) {
                    if(response.isSuccessful){
                        ...
                        if (result?.isSend == true){
                            ...
                            Toast.makeText(this@EmailActivity, "이메일을 전송했습니다.", Toast.LENGTH_LONG).show()
                        } else {
                            Toast.makeText(this@EmailActivity, "이메일 인증에 실패하였습니다.", Toast.LENGTH_LONG).show()
                        }
